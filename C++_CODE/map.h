...
map_node *create_map(
    int &count,
    vector<map_node *> &
        loc_node); //从“matrix.txt”中读取邻接矩阵并生成邻接表。返回头指针，头指针为空。
void appendNode(
    map_node *pcur,
    string
        name); //生成邻接表时，传入当前顶点地址和下一个顶点的名字，生成下一个顶点
void appendArc(
    map_node *head, ifstream &fmatrix, const int count,
    vector<map_node *> &
        loc_node); //生成邻接表时，输入当前顶点的地址，读入该顶点的所有边并生成相应的边节点
...
void appendArc(map_node *pcur_node, ifstream &fmatrix, const int count,
