...
nav_node* Min_dis_str(map_node * head,int count,map_node* begin,map_node* end,vector<map_node*>loc_node);//最短路径算法 使用dijkstra
nav_node *Min_time_str(map_node * head,int count,map_node* begin,map_node* end,vector<map_node*>loc_node);//最短时间算法 使用dijkstra
...
nav_node *Min_with_transport(map_node * head,int count,map_node* begin,map_node* end,vector<map_node*>loc_node);//最短骑单车的距离算法，使用dijkstra

nav_node * Min_weight_str(map_node * head,int count,map_node* begin,map_node* end,vector<map_node*>loc_node);//通过weight值来寻找weight_sum最短的路径
map_node * dij_find_next_node(map_node* head);//dijstra算法中，寻找没访问过的，且目前从起点到该点距离最短的点。
...
nav_node * Min_dis_str(map_node * head,int count,map_node* begin,map_node* end,vector<map_node*>loc_node){
    //使weight初始化为distance
...
nav_node * Min_time_str(map_node * head,int count,map_node* begin,map_node* end,vector<map_node*>loc_node){
...
nav_node * Min_weight_str(map_node * head,int count,map_node* begin,map_node* end,vector<map_node*>loc_node){
    nav_node* nav_head;
    ...
    for(int i=0;i<count;i++){
        ...
        while(pcur_arc){
            ...
            if((cur_weight_sum<pcur_arc->arc_node->sum_weight)&&pcur_arc!=0){//如果途经当前节点的路径比原路径更短
